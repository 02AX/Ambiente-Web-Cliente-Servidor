...
class DenunciaController {
  ...
  async getDenunciasOrdenadas(criterio = 'likes') {
    const denunciasCol = collection(db, 'denuncias');
    if (criterio === 'likes') {
      const qByLikes = query(denunciasCol, orderBy('likes', 'desc'));
      const snap = await getDocs(qByLikes);
      return snap.docs.map(doc => ({ id: doc.id, ...this._convertTimestamps(doc.data()) }));
    }
    if (criterio === 'fecha') {
      const qByDate = query(denunciasCol, orderBy('fechaCreacion', 'desc'));
  ...
  async getComentariosByDenuncia(denunciaId) {
    ...
    try {
      ...
      // Fallback when a composite index is required but not yet created
      const qNoOrder = query(comentariosCol, where('denunciaId', '==', denunciaId));
